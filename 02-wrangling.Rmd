...
<!-- 0) Fix all .groups messages. And other messages and warnings. -->

<!-- 1) "Data Gathering" (?) Download and Reading In (mention col_types, and how to make the message go away) Details on read_csv options. Also using skimr::skim() -->

<!-- 2) Characters. Need to update regexp to mention new string constant in R 4.0 at the beginning, and then delete the old.  -->

<!-- 3)  Factors -->

<!-- 4) Dates and Times -->

<!-- 5) Lists (vectors and vector manipulation? indexing? [], [[]]) -->

<!-- 6) "Combining Data" -->

<!-- 7) "Tidy Data" -->

<!-- 8) "Other Commands" missing values -->

<!-- 8a) Random variables: runif(), rbinom() and rnorm(). Showing differnt ways of calling them. Then plotting it when we call them a bunch. And then explaining what those histograms mean. (Look at chapter 5 for inspiration.) -->
...
<!-- Do we really need all these data packages? -->
...
Load the `tidyverse`, which includes `stringr`. The functions included in `stringr` begin with `str_`.
...
To manipulate factors we will use the [forcats][forcats-web] package, a core package in the tidyverse. Like the stringr package whose functions begin with `str_`, main functions of the forcats package start with `fct_`.
...
## Lists
<!-- EC: Added this section from R4DS -->
...
We will manipulate date-times using the lubridate package, which makes it easier to work with dates and times in R. `lubridate` is not part of core tidyverse because you only need it when you're working with dates/times.
...
Time zones are an enormously complicated topic because of their interaction with geopolitical entities. Fortunately we don't need to dig into all the details as they're not all important for data analysis. You can see the complete list of possible timezones with the function `OlsonNames()`. Unless otherwise specified, lubridate always uses UTC (Coordinated Universal Time).
...
<!-- DK: Maybe we put this down below, after we introduce rnorm()? -->
...
<!-- EC: Is using this moderndive paragraph enough background for distributions? DK: Yes. Vivian goes into the deep detail. This is enough. IN fact, we could even cut it.-->
...
<!-- EC: What formulas do we want? I'm guessing not something as complex as a probability density function, will explaining the elements of a normal/Uniform dist be enough? DK: No formulas in this chapter are needed, although if you wanted them in a side note you could. We only need to show histograms, as you have done, and discuss sensible scenarios for the use of each function. -->

The function `rnorm()` can be read as "r-norm". It corresponds to a Normal distribution, whose histograms look roughly like the following:

<!-- EC: Clean up graphic (or even just insert as picture?) DK: Maybe just use the graphic above? We certainly don't want to "insert" pictures. We generate what we want to show. -->
...
<!-- DK: hist() is too ugly. Use the full ggplot() approach -->

```{r}
distrib <- data.frame(value=rnorm(10))
...
```{r}
distrib_2 <- data.frame(value = rnorm(100))
...
```{r}
distrib_3 <- data.frame(value = rnorm(1000))
...
The function `runif()` can be read as "r-unif". It corresponds to a Uniform distribution, in which every outcome has the same chance of occurring. A Uniform distribution you might encounter everyday is rolling a fair dice. You are as likely to roll a 2 as you are to roll a 6.
...
<!-- DK: We want to show and explain rnorm(), runif() and rbinom(), with a clear explanation of coin-tossing as an example. Like how many heads might appear of you flip a coint with p = 0.6 20 times. (Use that exact example.) -->
